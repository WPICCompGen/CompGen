...
write.table(listFilters(ensembl),"ensembl.filters.txt",row.names=F,col.names=F,quote=F,sep="\t")
# this lists all fields you can put out
write.table(listAttributes(ensembl),"ensembl.attributes.txt",row.names=F,col.names=F,quote=F,sep="\t")
...
gene.info=getBM(attributes=c("ensembl_gene_id","ensembl_transcript_id","ensembl_exon_id","exon_chrom_start","exon_chrom_end","hgnc_symbol"),
                filters="ensembl_gene_id",values=gene.id,mart=ensembl)
# you name get an error stating that you are selecting from multiple attribute pates
...
gene.info=getBM(attributes=c("ensembl_gene_id","hgnc_id"),
